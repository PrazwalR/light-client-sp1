...
### Real Core Proof (local CPU, requires 8+ GB RAM)

```bash
SP1_PROVER=cpu RUST_LOG=info cargo run --release -- --prove --live
```

> **Note:** SP1 v6 (Hypercube) renamed `local` to `cpu`. The code auto-remaps
> `SP1_PROVER=local` → `cpu`, but using `cpu` directly is recommended.
...
### Groth16 Proof (Local CPU — real proof)

```bash
SP1_PROVER=cpu RUST_LOG=info cargo run --release --bin evm -- --system groth16
```
...
### PLONK Proof (Local CPU — real proof)

```bash
SP1_PROVER=cpu RUST_LOG=info cargo run --release --bin evm -- --system plonk
```
...
## 14b. Fixture-Based Tests (EVM Proof Validation)

After generating EVM proofs with `cargo run --release --bin evm`, the generated fixture JSON files
can be tested against the Solidity contract to validate the full on-chain flow.

### Generated Files

After running `--bin evm`, you'll find these in `contracts/src/fixtures/`:

| File | Generated By |
|------|-------------|
| `groth16-fixture.json` | `--system groth16` |
| `plonk-fixture.json` | `--system plonk` |

Each fixture contains: `finalizedSlot`, `finalizedHeaderRoot`, `finalizedStateRoot`,
`currentSyncCommitteeHash`, `nextSyncCommitteeHash`, `participation`, `vkey`,
`publicValues` (ABI-encoded), and `proof` (hex bytes).
...
forge test -vvv --match-contract SP1FixtureTest

# Run specific fixture tests
forge test -vvv --match-test test_groth16_fixture_full_flow
forge test -vvv --match-test test_plonk_fixture_full_flow
forge test -vvv --match-test test_groth16_decode_public_values
forge test -vvv --match-test test_simulate_multi_update_flow
forge test -vvv --match-test test_gas_estimation_update

# Run with gas report
forge test -vvv --match-contract SP1FixtureTest --gas-report
```
...
|------|------------------|
| `test_groth16_fixture_loads` | Fixture JSON parses correctly, vkey/slot/participation match |
| `test_groth16_fixture_full_flow` | Deploy → initialize → submit proof → verify all state updated |
| `test_plonk_fixture_loads` | Same as above for PLONK fixture |
| `test_plonk_fixture_full_flow` | Full PLONK flow: deploy → init → proof → verify |
| `test_groth16_decode_public_values` | ABI-decode publicValues and verify individual fields |
| `test_simulate_multi_update_flow` | 3 consecutive updates with sync committee rotation |
| `test_gas_estimation_update` | Gas cost estimation for `update()` call |

### Mock vs Real Proofs

| Prover | `proof` field | On-chain verification? | Testnet deploy? |
|--------|--------------|----------------------|-----------------|
| `SP1_PROVER=mock` | `0x` (empty) | MockVerifier only (local Foundry) | No |
| `SP1_PROVER=cpu` | Real Groth16/PLONK bytes | Yes, via SP1Verifier | Yes |
| `SP1_PROVER=cuda` | Real Groth16/PLONK bytes (GPU) | Yes, via SP1Verifier | Yes |
| `SP1_PROVER=network` | Real Groth16/PLONK bytes | Yes, via SP1Verifier | Yes |

> **Important:** Mock proofs (`proof: "0x"`) can only be verified locally with a MockVerifier.
> For actual on-chain deployment and verification, you need real proofs generated with
> `SP1_PROVER=cpu` (requires 16+ GB RAM) or `SP1_PROVER=network` (Succinct's prover network).
>
> **Note:** SP1 v6 renamed `SP1_PROVER=local` to `SP1_PROVER=cpu`. Using `local` will
> auto-remap to `cpu` with a deprecation warning.
...
[!] Proof verification skipped (SP1_PROVER=mock generates stub proofs)
    Use SP1_PROVER=cpu or SP1_PROVER=network for real verification.
```
...
SP1_PROVER=cpu cargo run --release -- --prove --live
...
SP1_PROVER=cpu cargo run --release --bin evm -- --system groth16 --live
SP1_PROVER=cpu cargo run --release --bin evm -- --system plonk --live
...
# === FIXTURE TESTS (after generating EVM proofs) ===
cd ../contracts && forge test -vvv --match-contract SP1FixtureTest
